...
def obtain_loader(dataset_name, train_file, val_file, test_file):
    ...
    data, label = torch.load(f'../LMs/encoded_data/bert/original_{dataset_name}.pt')
    extractive = torch.load(f'representations/abstractive_original_{dataset_name}.pt')
    abstractive = torch.load(f'representations/abstractive_original_{dataset_name}.pt')

    train_indices = json.load(open(train_file))
    val_indices = json.load(open(val_file))
    test_indices = json.load(open(test_file))

    dataset = MyDataset(claims=data, labels=label, abstractive=abstractive,
