...
def obtain_shortcut_dataset(first, second):
    if first.find('original') != -1:
        first = first.split('/')[-1]
        first_data = json.load(open(f'../../datasets/{first}.json'))
    else:
        first_data = json.load(open(f'../../extrinsic/data/{first}.json'))
    if second.find('original') != -1:
        second = second.split('/')[-1]
        second_data = json.load(open(f'../../datasets/{second}.json'))
    else:
        second_data = json.load(open(f'../../extrinsic/data/{second}.json'))
