...
if not os.path.exists(f'{debias}_res'):
    os.mkdir(f'{debias}_res')
...
def main():
    ...
    for index, category in enumerate(['llm-generation', 'sentiment', 'word-choice', 'tone', 'age', 'gender']):
        ...
        res_path = f'{debias}_res/{lm}_{category}.json'
        ...
        for dataset_name in dataset_names:
            for fake in candidate:
                for real in candidate:
                    ...
                    res = obtain(f'{debias}_{category}_{dataset_name}_{fake}',
                                 f'{debias}_{category}_{dataset_name}_{real}')
