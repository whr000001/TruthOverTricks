...
def main():
    ...
    for dataset in datasets:
        ...
        for profile in ['young', 'elder']:
            res = []
            save_path = f'../datasets/age/{dataset}_{profile}.json'
            if os.path.exists(save_path):
                continue
            for item in data:
                claim = item['claim']
                claim = construct_length(claim)
                prompt = 'You are {}. Given a passage, please rewrite it without any explanations. ' \
                         'The content should be the same. The passage is: {}'.format(profile, claim)
